...
const LanguageAccessibility = () => {
  ...
  const { language, setLanguage, t, textSize, setTextSize, getFontSize, fontType, setFontType, fontFamily } = useLanguage();
    ...
    return (
      <View style={{ flex: 1, backgroundColor: colors.background, padding: 40 }}>
        ...
        <View style={{ maxWidth: 600, alignSelf: 'center', width: '100%', gap: 30 }}>
            {/* Langue */}
            <View style={{ gap: 10 }}>
                <Text style={{ fontSize: 18, fontWeight: '600', color: colors.text, fontFamily }}>{t('language')}</Text>
            ...
            {/* Taille texte */}
            <View style={{ gap: 10 }}>
                <Text style={{ fontSize: 18, fontWeight: '600', color: colors.text, fontFamily }}>{t('textSize')}</Text>
            ...
            {/* Police */}
            <View style={{ gap: 10 }}>
                <Text style={{ fontSize: 18, fontWeight: '600', color: colors.text, fontFamily }}>{t('font')}</Text>
                <View style={{ borderWidth: 1, borderColor: '#DDD', borderRadius: 8, backgroundColor: colors.card }}>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <Text style={[styles.pageTitle, { fontSize: getFontSize(20), color: colors.text, fontFamily: fontFamily, marginBottom: 30 }]}>
          {t('languageAccessibility')}
        </Text>

        <View style={styles.formContainer}>
          <View style={styles.fieldGroup}>
            <Text style={[styles.label, { fontSize: getFontSize(16), color: colors.text, fontFamily: fontFamily }]}>{t('language')} :</Text>
          ...
          <View style={styles.fieldGroup}>
            <Text style={[styles.label, { fontSize: getFontSize(16), color: colors.text, fontFamily: fontFamily }]}>{t('textSize')} :</Text>
          ...
          <View style={styles.fieldGroup}>
            <Text style={[styles.label, { fontSize: getFontSize(16), color: colors.text, fontFamily: fontFamily }]}>{t('font')} :</Text>
            <View style={[styles.pickerContainer, { backgroundColor: colors.card, borderColor: colors.border }]}>
